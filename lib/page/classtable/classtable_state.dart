...
class ClassTableWidgetState with ChangeNotifier {
  ...
  set isPartner(bool isPartner) {
    if (isPartner && partnerClass == null) {
      return;
    }
  ...
  bool get havePartner => partnerClass != null;
    ...
    for (var i in timeArrangement) {
      ...
      for (j; j < i.weekList.length; j++) {
        if (i.weekList[j] && start == -1) {
          start = j;
          lastClassroom = i.classroom![(j + 1).toString()] ?? "";
        } else if (!i.weekList[j] &&
            start != -1 &&
            lastClassroom != i.classroom![(j + 1).toString()]) {
      ...
      List<String> startTime = i.isWinter
          ? winterTime[(i.start - 1) * 2].split(":")
          : summerTime[(i.start - 1) * 2].split(":");
      List<String> stopTime = i.isWinter
          ? winterTime[(i.stop - 1) * 2 + 1].split(":")
          : summerTime[(i.stop - 1) * 2 + 1].split(":");
      ...
      for (var range in ranges) {
        String title =
            "${getClassDetail(timeArrangement.indexOf(i)).name}@${i.classroom?[(range.$1 + 1).toString()] ?? "待定"}";
        String? location = i.classroom?[(range.$1 + 1).toString()] ?? "待定";
...
''';
  ...
  List<ClassOrgainzedData> getArrangement({
    required int weekIndex, // 第几周，从 0 开始
    required int dayIndex, // 第几天，从 1 开始
  }) {
