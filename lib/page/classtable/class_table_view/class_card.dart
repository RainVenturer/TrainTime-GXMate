...
class ClassCard extends StatelessWidget {
  ...
  Map<String, String>? get place => detail.place;
  const ClassCard({
    super.key,
                    ...
                    onPressed: () async {
                      ...
                      (ClassDetail, TimeArrangement, bool)? toUse =
                        ...
                        child: ArrangementDetail(
                          ...
                          classTableState: classTableState, // 新增这一行
                        ),
                        context: context,
                      );
                      ...
                      children: [
                        Text(
                          ...
                          style: TextStyle(
                            ...
                            fontSize: isPhone(context) ? 12 : 14, // 减小字体
                          ),
                        ),
                        if ((detail.stop - detail.start) >= 2) // 只在高度足够时显示教室信息
                          Text(
                            "@${place?[(classTableState.chosenWeek + 1).toString()] == null || place?[(classTableState.chosenWeek + 1).toString()] == "" ? FlutterI18n.translate(
                                context,
                                "classtable.class_card.unknown_classroom",
                              ) : place?[(classTableState.chosenWeek + 1).toString()]}",
                            style: TextStyle(
                              color: color.shade900,
                              fontSize: isPhone(context) ? 10 : 12, // 减小字体
                            ),
                          ).expanded(),
                        if (data.length > 1 &&
                            (detail.stop - detail.start) >=
                                3) // 只在高度足够时显示剩余课程信息
                          Text(
