...
JWSLoginState loginState = JWSLoginState.none;

bool get offline =>
    loginState != JWSLoginState.success && loginState != JWSLoginState.manual;

class JWSSession extends NetworkSession {
      ...
      for (int i = 0; i < retryCount; i++) {
        ...
        if (!dataJson.containsKey('tgt')) {
          ...
          var redirect = await dioNoOfflineCheck.get(
            target != null
                ? "$target/new/ssoLogin?ticket=${dataJson['ticket']}"
                : "http://portal.gxmu.edu.cn?ticket=${dataJson['ticket']}",
            options: Options(
              headers: {
